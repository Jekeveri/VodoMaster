import flet as ft
import uuid
from flet import (
    IconButton, ElevatedButton, TextField, Container, Column, Row,
    MainAxisAlignment, BoxShadow, padding, colors, icons, ButtonStyle, FontWeight, ScrollMode,
    alignment
)

import src.database.admin.delete_server as delete_server
import src.database.admin.select_server as select_server
from src.ui.components.crud_dialogs.create_task_dialog import CreateTaskDialog
from src.ui.components.crud_dialogs.create_task_load_dialog import AddNewTaskDialog
from src.ui.components.table_components.class_table import FilterableDataTable
from src.ui.components.table_components.data_tuples import load_data_from_tuples
from src.ui.components.table_components.page_setting import PagePanel
from src.ui.utils.ui_blocker import block_ui
from src.ui.utils.navigation import nav_manager
from src.ui.utils.action_menu import PopupMenuUp


def meters_tab(page):
    @block_ui(page)
    def load_data():
        return select_server.select_task_data_all()

    data_server = load_data()

    # Явно задаём порядок полей, которые нас интересуют
    FIELDS_ORDER = [
        'meter_number',
        'meter_remark',
        'installation_date',
        'type_water',
        'marka',
        'next_verification',
        'location',
        'antimagnetic_protection',
        'average_consumption',
        'month_not_reading',
        'meter_end_date',
        'seal_number',
        'seal_installation_date',
        'last_reading_value',
        'last_reading_date'
    ]

    all_meters = []

    for task in data_server:
        if isinstance(task, tuple) and len(task) > 8:  # Проверяем, что есть нужные поля
            # Извлекаем данные адреса из задачи
            street = task[5]  # Улица
            settlement = task[6]  # Поселок
            house = task[7]  # Дом
            apartment = task[8]  # Квартира
            address_id = task[2]

            meters_data = task[-3]

            if isinstance(meters_data, list):
                for meter in meters_data:
                    if isinstance(meter, dict):
                        # Добавляем адресные данные к данным счетчика
                        values = [
                                     meter.get(field)
                                     for field in FIELDS_ORDER
                                 ] + [street, settlement, house, apartment, address_id]

                        all_meters.append(tuple(values))

    data = load_data_from_tuples(
        data_tuples=all_meters,
        exclude_columns=[],
        columns=[
            'Номер счетчика',
            'Замечание по счетчику',
            'Дата установки',
            'Тип',
            'Марка',
            'Следующая проверка',
            'Расположение',
            'Антимагнитный',
            'Среднее_потребление',
            'Месяцев не проверяли',
            'Дата выхода из службы',
            'Номер пломбы',
            'Дата установки пломбы',
            'Данные последней проверки',
            'Дата последней проверки',
            'Улица',
            'Поселок',
            'Дом',
            'Квартира',
            'address_id'
        ],
        date_columns=['Дата установки', 'Дата последней проверки', 'Дата выхода из службы', 'Дата установки пломбы'],
        numeric_columns=['Среднее_потребление', 'Месяцев не проверяли', 'address_id'],
        string_columns=[
            'Номер счетчика', 'Замечание по счетчику', 'Тип', 'Марка', 'Данные последней проверки', 'Номер пломбы',
            'Расположение', 'Антимагнитный', 'Следующая проверка', 'Поселок', 'Дом', 'Квартира'
        ],
        json_columns=[]
    )

    def perform_search(e):
        search_term = search_field.value.lower() if search_field.value else ""
        table.apply_search_filter(search_term)

        if hasattr(search_field.suffix_icon, 'visible'):
            search_field.suffix_icon.visible = bool(search_term)
        elif isinstance(search_field.suffix_icon, ft.IconButton):
            search_field.suffix_icon.visible = bool(search_term)

        # Принудительное обновление пагинации
        table._update_pagination_controls()
        if table.page:
            table.page.update()

    def reload_page(e):
        nonlocal data, table

        @block_ui(page)
        def load_data():
            return select_server.select_task_data_all(refresh=True)

        data_server = load_data()

        # Явно задаём порядок полей, которые нас интересуют
        FIELDS_ORDER = [
            'meter_number',
            'meter_remark',
            'installation_date',
            'type_water',
            'marka',
            'next_verification',
            'location',
            'antimagnetic_protection',
            'average_consumption',
            'month_not_reading',
            'meter_end_date',
            'seal_number',
            'seal_installation_date',
            'last_reading_value',
            'last_reading_date'
        ]

        all_meters = []

        for task in data_server:
            if isinstance(task, tuple) and len(task) > 8:  # Проверяем, что есть нужные поля
                # Извлекаем данные адреса из задачи
                street = task[5]  # Улица
                settlement = task[6]  # Поселок
                house = task[7]  # Дом
                apartment = task[8]  # Квартира
                address_id = task[2]

                meters_data = task[-3]
                if isinstance(meters_data, list):
                    for meter in meters_data:
                        if isinstance(meter, dict):
                            # Добавляем адресные данные к данным счетчика
                            values = [
                                         meter.get(field)
                                         for field in FIELDS_ORDER
                                     ] + [street, settlement, house, apartment, address_id]

                            all_meters.append(tuple(values))

        data = load_data_from_tuples(
            data_tuples=all_meters,
            exclude_columns=[],
            columns=[
                'Номер счетчика',
                'Замечание по счетчику',
                'Дата установки',
                'Тип',
                'Марка',
                'Следующая проверка',
                'Расположение',
                'Антимагнитный',
                'Среднее_потребление',
                'Месяцев не проверяли',
                'Дата выхода из службы',
                'Номер пломбы',
                'Дата установки пломбы',
                'Данные последней проверки',
                'Дата последней проверки',
                'Улица',
                'Поселок',
                'Дом',
                'Квартира',
                'address_id'
            ],
            date_columns=['Дата установки', 'Дата последней проверки', 'Дата выхода из службы',
                          'Дата установки пломбы'],
            numeric_columns=['Среднее_потребление', 'Месяцев не проверяли', 'address_id'],
            string_columns=[
                'Номер счетчика', 'Замечание по счетчику', 'Тип', 'Марка', 'Данные последней проверки', 'Номер пломбы',
                'Расположение', 'Антимагнитный', 'Следующая проверка', 'Поселок', 'Дом', 'Квартира''address_id'
            ],
            json_columns=[]
        )

        table.original_df = data
        table.filtered_df = data.copy()
        table.filter_settings.clear()
        table.current_page = 0
        table.selected_rows.clear()

        # Принудительно обновляем таблицу
        table._update_table()
        # table._update_filter_indicator()
        table._update_selection_count_indicator()

        # Обновляем панель фильтров, если она существует
        if hasattr(table, 'page_panel'):
            # Используем правильное имя метода с подчеркиванием
            table.page_panel._reset_filters()

        # Обновляем страницу
        if hasattr(page, 'update'):
            page.update()

    def clear_search(e):
        search_field.value = ""

        if isinstance(search_field.suffix_icon, ft.IconButton):
            search_field.suffix_icon.visible = False

        perform_search(e)

    def open_load_dialog(e):
        dialog = AddNewTaskDialog(page)

        # Создаем AlertDialog с содержимым UserControl
        excel_dialog = ft.AlertDialog(
            title=ft.Text("Загрузка задач из Excel"),
            content=dialog,
            actions=[
                ft.TextButton("Закрыть", on_click=lambda e: dialog.close_dialog(e),
                              style=ft.ButtonStyle(
                                  bgcolor=ft.colors.BLUE_600,
                                  color=ft.colors.WHITE
                              ))
            ],
            bgcolor=ft.colors.BLUE_50,
            surface_tint_color=ft.colors.BLUE_50,
            elevation=5
        )

        # Устанавливаем диалог и открываем его
        page.dialog = excel_dialog
        page.dialog.open = True
        page.update()

    def create_task_button(e=None):
        task_dialog = CreateTaskDialog(page)
        task_dialog.open()

    def show_filter_panel(e):
        filter_panel.show()

    def create_animated_container(content, **kwargs):
        return Container(
            content=content,
            animate=ft.animation.Animation(300, curve=ft.animation.AnimationCurve.EASE_OUT),
            **kwargs
        )

    def create_action_button(text, on_click, icon, color=colors.BLUE, disabled=False):
        return ElevatedButton(
            content=Row(
                [
                    ft.Icon(icon, color=colors.WHITE),
                    ft.Text(text, color=colors.WHITE, weight=FontWeight.W_500)
                ],
                alignment=MainAxisAlignment.CENTER,
                spacing=5,
            ),
            style=ButtonStyle(
                shape=ft.RoundedRectangleBorder(radius=8),
                bgcolor=color,
                elevation=5,
            ),
            on_click=on_click,
            disabled=disabled,
        )

    search_field = TextField(
        label="Поиск",
        expand=True,
        prefix_icon=icons.SEARCH,
        border_color=colors.BLUE,
        focused_border_color=colors.BLUE,
        cursor_color=colors.BLUE,
        on_change=perform_search,
        suffix_icon=ft.IconButton(
            icon=icons.CLOSE,
            on_click=clear_search,
            icon_size=18,
            visible=False,  # Изначально скрыта
            style=ButtonStyle(
                padding=padding.all(0),
                overlay_color=colors.TRANSPARENT,
            )
        ),
    )

    action_menu = ft.Column(
        [],
        spacing=0,
        visible=False,
        left=100,  # Настройте позицию
        top=-120,  # Смещение вверх
        animate_opacity=300,
        animate_scale=300,
    )

    button_edit = Row(
        [
            create_action_button("Подробнее адреса", lambda _: further_meters_button(), icons.INFO),
        ],
        alignment=MainAxisAlignment.CENTER,
        spacing=10,
        visible=False,  # По умолчанию скрыты
    )

    menu_stack = ft.Stack(
        controls=[],
        height=40  # Высота кнопки меню
    )

    menu_button = ft.IconButton(
        icon=ft.icons.MORE_VERT,
        icon_size=28,
        icon_color=colors.BLUE_GREY_600,
        tooltip="Дополнительные действия",
        on_click=lambda e: toggle_menu(e),
        style=ft.ButtonStyle(padding=padding.all(8.0)),
    )

    # 3. Контейнер для меню
    menu_container = ft.Container(
        content=ft.Column(
            [],
            spacing=0,
            visible=False,
        ),
        border=ft.border.all(1, ft.colors.GREY_300),
        bgcolor=ft.colors.WHITE,
        shadow=ft.BoxShadow(
            spread_radius=1,
            blur_radius=5,
            color=ft.colors.BLUE_GREY_300,
            offset=ft.Offset(0, 0)
        ),
        border_radius=5,
        padding=ft.padding.only(top=5, bottom=5),
        top=0,  # Смещение вверх относительно кнопки
        left=0,
    )

    # menu_stack.controls.extend([menu_button, menu_container])
    #
    # page.overlay.append(menu_stack)

    def toggle_menu(e):
        menu_container.content.visible = not menu_container.content.visible
        menu_container.top = -120
        menu_container.left = 40
        page.update()

    def update_action_buttons(selected_rows):
        num_selected = len(selected_rows)
        button_edit.visible = num_selected == 1
        items = []

        if num_selected == 1:
            items.append({
                "text": "Редактировать",
                "on_click": lambda e: update_task_button(),
                "icon": ft.Icon(ft.icons.EDIT, color=ft.colors.ORANGE_600)
            })

        if num_selected >= 1:
            items.append({
                "text": f"Удалить счетчик(и)",
                "on_click": lambda e: delete_task_button(),
                "icon": ft.Icon(ft.icons.DELETE, color=ft.colors.RED_600)
            })

        menu.set_items(items)
        page.update()

    def update_task_button():
        # Placeholder for task update logic
        page.show_snack_bar(
            ft.SnackBar(
                content=ft.Text("Функция редактирования в разработке"),
                duration=2000
            )
        )

    def further_meters_button():
        selected_ids = table.filtered_df.loc[list(table.selected_rows), 'address_id'].tolist()
        result = selected_ids[0]

        new_buttons = [
            {
                "icon": ft.icons.INFO,
                "label": "Детали адреса",
                "data": "address_details",
                "id": f"address_details_{uuid.uuid4()}",
                "additional_data": {"employee_id": result},
            }
        ]
        nav_manager.add_nav_button_and_navigate(new_buttons[0])

    def delete_task_button():
        mass_ids = table.filtered_df.loc[list(table.selected_rows), 'ID'].tolist()

        # Создание диалога подтверждения
        def confirm_delete(e):
            confirm_dialog.open = False
            page.update()
            delete_server.delete_tasks(mass_ids)

        def cancel_delete(e):
            confirm_dialog.open = False
            page.update()

        confirm_dialog = ft.AlertDialog(
            modal=True,
            title=ft.Text(
                "Удаление задач",
                size=24,
                weight=ft.FontWeight.BOLD,
                color=ft.colors.RED_600
            ),
            content=ft.Column([
                ft.Row([
                    ft.Icon(ft.icons.WARNING_ROUNDED, color=ft.colors.RED_600, size=48),
                    ft.Text(
                        f"Вы действительно хотите удалить {len(mass_ids)} задач(и)?",
                        size=16,
                        color=ft.colors.GREY_800
                    )
                ], alignment=ft.MainAxisAlignment.CENTER, spacing=15),
                ft.Text(
                    "Восстановить удаленные задачи будет невозможно.",
                    color=ft.colors.RED_600,
                    italic=True,
                    size=14
                )
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER, height=150),
            actions=[
                ft.TextButton(
                    "Отмена",
                    on_click=cancel_delete,
                    style=ft.ButtonStyle(color=ft.colors.GREY_700)
                ),
                ft.FilledButton(
                    "Удалить",
                    on_click=confirm_delete,
                    style=ft.ButtonStyle(
                        bgcolor=ft.colors.RED_600,
                        color=ft.colors.WHITE
                    )
                )
            ],
            actions_alignment=ft.MainAxisAlignment.END,
            bgcolor=ft.colors.WHITE,
            shape=ft.RoundedRectangleBorder(radius=10)
        )

        page.dialog = confirm_dialog
        confirm_dialog.open = True
        page.update()

    def assign_tasks():
        # Placeholder for task assignment logic
        mass_ids = table.filtered_df.loc[list(table.selected_rows), 'ID'].tolist()
        page.show_snack_bar(
            ft.SnackBar(
                content=ft.Text(mass_ids),
                duration=2000
            )
        )

    top_bar = create_animated_container(
        Column([
            Row(
                [
                    search_field,
                    create_animated_container(
                        IconButton(
                            icon=icons.SEARCH,
                            tooltip="Поиск",
                            on_click=perform_search,
                            style=ButtonStyle(color=colors.WHITE, shape=ft.CircleBorder()),
                            icon_size=28,
                        ),
                        width=50,
                        height=50,
                        bgcolor=colors.BLUE_600,
                        border_radius=ft.border_radius.all(15),
                        alignment=alignment.center,
                    ),
                    create_animated_container(
                        IconButton(
                            icon=icons.REFRESH,
                            tooltip="Обновить",
                            on_click=reload_page,
                            style=ButtonStyle(color=colors.WHITE, shape=ft.CircleBorder()),
                            icon_size=28,
                        ),
                        width=50,
                        height=50,
                        bgcolor=colors.BLUE_GREY_600,
                        border_radius=ft.border_radius.all(15),
                        alignment=alignment.center,
                    ),
                    create_animated_container(
                        IconButton(
                            icon=icons.FILTER_LIST,
                            tooltip="Фильтры",
                            on_click=lambda e: show_filter_panel(e),
                            style=ButtonStyle(color=colors.WHITE, shape=ft.CircleBorder()),
                            icon_size=28,
                        ),
                        width=50,
                        height=50,
                        bgcolor=colors.BLUE_GREY_600,
                        border_radius=ft.border_radius.all(15),
                        alignment=alignment.center,
                    ),
                ],
                alignment=MainAxisAlignment.END,
                spacing=12,
            )
        ]),
        border_radius=15,
        bgcolor=colors.WHITE,
        margin=5,
        padding=padding.symmetric(horizontal=20, vertical=18),
        shadow=BoxShadow(
            color=colors.BLUE_GREY_300,
            offset=ft.Offset(2, 0),
            blur_radius=5,
            spread_radius=1,
        ),
    )

    columns_config = {
        "Номер счетчика": {"type": "category"},
        "Поселок": {"type": "category"},
        "Улица": {"type": "category"},
        "Дом": {"type": "category"},
        "Квартира": {"type": "numeric"},
        "Замечание по счетчику": {"type": "category"},
        "Дата установки": {"type": "date"},
        "Тип": {"type": "category"},
        "Марка": {"type": "category"},
        "Следующая проверка": {"type": "date"},
        "Расположение": {"type": "category"},
        "Антимагнитный": {"type": ""},
        "Среднее_потребление": {"type": "numeric"},
        "Месяцев не проверяли": {"type": "numeric"},
        "Дата выхода из службы": {"type": "date"},
        "Номер пломбы": {"type": "category"},
        "Дата установки пломбы": {"type": "date"},
        "Данные последней проверки": {"type": "numeric"},
        "Дата последней проверки": {"type": "date"},
        "address_id": {"type": "numeric"}
    }

    filter_panel = PagePanel(page, page_type="meters", search_field=search_field)
    table = FilterableDataTable(
        data,
        columns_config,
        hidden_columns=["Антимагнитный", "Среднее_потребление", "Дата установки пломбы", "address_id"],
        on_selection_change=update_action_buttons,
        page_type="meters",
        search_field=search_field
    )
    table.set_page(page)
    filter_panel.set_table(table)
    table.page_panel = filter_panel

    # В функции search_tab заменить создание table_container
    table_container = Container(
        content=Column([
            table.selection_count_indicator,
            table.filter_indicator,
            Row([table.data_table], scroll=ScrollMode.AUTO)
        ], scroll=ScrollMode.AUTO),  # Добавляем горизонтальный скролл
        expand=True,
        padding=10
    )

    def on_click(e):
        page.snack_bar = ft.SnackBar(ft.Text("Пункт выбран"))
        print("Пункт выбран")
        page.snack_bar.open = True
        page.update()

    menu = PopupMenuUp([], page)

    content = Column([
        top_bar,
        table_container,
    ], expand=True)

    bottom_panel = Container(
        content=Row(
            [
                Column(
                    [
                        Row(
                            [
                                # create_action_button(
                                #     "Добавить",
                                #     lambda _: print(123),
                                #     icons.ADD,
                                #     colors.GREEN_600,
                                # ),
                                table.add_pagination_controls()
                            ]
                        )
                    ],
                    alignment=MainAxisAlignment.CENTER,
                ),
                Row(
                    [
                        button_edit,
                        # menu.get_view()
                    ],
                    alignment=MainAxisAlignment.CENTER,
                ),
            ],
            alignment=MainAxisAlignment.SPACE_BETWEEN,
            spacing=20,
        ),
        padding=padding.symmetric(horizontal=15, vertical=12),
        bgcolor=colors.WHITE,
        border_radius=12,
        shadow=BoxShadow(
            color=colors.BLUE_GREY_300,
            offset=ft.Offset(0, -2),
            blur_radius=5,
            spread_radius=1,
        ),
    )

    return Container(
        content=Column([
            content,
            bottom_panel,
        ], ),
        expand=True,
    )
