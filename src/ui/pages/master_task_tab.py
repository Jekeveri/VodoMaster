...
def master_task_tab(page):
    current_user_id = session_manager.get_current_user_id()
    if not current_user_id:
        show_snack_bar(page, "Сессия не найдена. Требуется авторизация!")
        return ft.Container()

    # Получаем данные сессии
    session_data = session_manager.get_session(current_user_id)
    if not session_data:
        show_snack_bar(page, "Ошибка авторизации!")
        return ft.Container()
    ...
    for task in data_server:
        if isinstance(task, tuple):
            # Извлекаем данные счетчиков (предполагаем, что они на позиции -3)
            meters_data = task[-3] if len(task) >= 3 else []
            locations = []
            if isinstance(meters_data, list):
                for meter in meters_data:
                    if isinstance(meter, dict):
                        loc = meter.get('location')
                        if loc:
                            locations.append(str(loc))
            location_str = ', '.join(locations)
            # Создаем новый кортеж с добавленным location
            modified_task = task + (location_str,)
            modified_data.append(modified_task)
        else:
            modified_data.append(task)

    filtered_data = []
    for task in modified_data:
        if isinstance(task, tuple) and len(task) > 23:  # Проверяем индекс Мастера
            if task[23] == current_user_login:  # Индекс 23 для столбца 'Мастер'
                filtered_data.append(task)

    data = load_data_from_tuples(
            data_tuples=filtered_data,
            exclude_columns=['id_квартиры', 'Счетчики', 'Акты', 'Фото'],
            columns=[
                ...
                'Сальдо', 'Исполнитель', 'Дата_выполнения', 'Мастер', 'Счетчики', 'Акты', 'Фото', 'Расположение'
            ],
            date_columns=['Дата', 'Дата_выполнения'],
            numeric_columns=['ID', 'id_квартиры', 'Нормативы', 'Квартира', 'Лицевой_счет'],
            string_columns=[
                'ФИО', 'Город', 'Район', 'Улица',
                'Поселок', 'Дом', 'Подъезд', 'Прописано',
                'Тип', 'Площадь', 'Телефон',
                'Комментарий', 'Статус', 'Причина',
                'Сальдо', 'Исполнитель', 'Расположение', 'Мастер'
            ],
            json_columns=['Счетчики', 'Акты', 'Фото']
        )
    ...
    button_edit = Row(
        [
            create_action_button("Подробнее", lambda _: further_task_button(), icons.INFO),
    ...
    def further_task_button():
        selected_ids = table.filtered_df.loc[list(table.selected_rows), 'ID'].tolist()
    ...
    table = FilterableDataTable(
        data,
        columns_config,
        hidden_columns=["Сальдо", "Город", "Район", "Подъезд", "Площадь", "Прописано", 'Мастер'],
        on_selection_change=update_action_buttons,
        page_type="master_task",
